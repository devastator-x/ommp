...
pub struct AlbumArtistsPane {
    pub selected: usize,
    pub scroll_offset: usize,
}

impl AlbumArtistsPane {
    pub fn new() -> Self {
        Self {
            selected: 0,
            scroll_offset: 0,
...
impl Pane for AlbumArtistsPane {
    fn render(&mut self, frame: &mut Frame, area: Rect, focused: bool, app: &App, theme: &Theme) {
        let album_artists = app.library.get_album_artists();
        ...
        let block = Block::default()
            ...
            .title(format!(" Album Artists ({}) ", album_artists.len()))
        ...
        let items: Vec<ListItem> = album_artists
            .iter()
            .enumerate()
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                if self.selected < count {
                    let tracks =
                        app.library.get_tracks_by_album_artist(&album_artists[self.selected]);
                    if !tracks.is_empty() {
    ...
    fn handle_mouse(&mut self, event: MouseEvent, area: Rect, app: &App) -> Option<AppAction> {
        ...
        let count = app.library.get_album_artists().len();
